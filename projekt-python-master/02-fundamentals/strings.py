...
Použijte kombina)ci různý možností pro práci s řetězci (včetně různých funkcí) i jiných prvků jazyka Python 
...
#1.
"""date = input("zadej datum DD. MM. RRRR\n")
date = date.replace(" ", "")
print(f"{date[-4:]}-{date[-7:-5]}-{date[0:2]}")
"""
#2. 
   #a)
def a(txt): 
...
def hesla(pocet):
    for i in range(pocet):
        cast1 = ''.join([chr(random.randint(65, 90)) for i in range(3)])
        cast2 = ''.join([chr(random.randint(97, 122)) for i in range(3)])
        cast3 = "-+*"[random.randint(0, 2)]
        cast4 = ''.join([str(random.randint(0, 9)) for i in range(3)])
        print(cast1 + cast2 + cast3 + cast4)
